...
If you haven't installed the extension, here are some examples of what a annotated website will look like:
...
* Couldn't get a `.tech` domain, even through GitHub Student Pack; We pivoted into using ngrok which required custom headers on every request.
  ...
  * We would have loved `deporia.tech`: short & easy to pronounce
...
* SHA256 hash matching requires exact pixel match - minor edits/reencodin create new hashes.
...
* Very simple and low-cogntiive-load UI
...
* Crowdsourcing works but needs critical mass of users
  * We believe that since "early adopters" tend to be more technical, they may also be better at identifying images made with AI, boosting the initial score quality
  * Whenever the extension sees an image for the first time, it marks it as "somewhat trustworthy" which may cause a user to put trust into a publication that is manipuated
...
* ML model to provide initial reputation score before community votes, currently it starts out at a flat "70%" or "somewhat trustworthy"
  ...
  * This is how some of our inspirations work
